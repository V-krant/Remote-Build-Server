...
namespace MessagePassingComm
{
  ...
  public class Receiver : IMessagePassingComm
  {
    ...
    public bool openFileForWrite(string name,string spath)
    {
            try
            {
                if (!System.IO.Directory.Exists(spath))
                    System.IO.Directory.CreateDirectory(spath);
                string writePath = Path.Combine(spath, name);
  ...
  public class Sender
  {
    ...
    public bool postFile(string fileName,string spath,string lpath)
    {
      FileStream fs = null;
      long bytesRemaining;
      ...
      try
      {
         ...
         string path = Path.Combine(lpath, fileName);                
        fs = File.OpenRead(path);
        bytesRemaining = fs.Length;
        channel.openFileForWrite(fileName,spath);
        while (true)
        {
